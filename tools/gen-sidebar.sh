#!/usr/bin/env bash
#
# Generate _sidebar.md for the Docsify site from the repo's folder layout.
#
#   <Difficulty>/<id>.<Title-With-Hyphens>/solution*.md
#
# Problems are grouped by difficulty and sorted by numeric problem id. A folder
# with a plain solution.md gets a single link; one with only variant files
# (solution-go.md, solution-bottomup.md, ...) gets a nested list of those.
#
# Run from anywhere:  ./tools/gen-sidebar.sh
#
set -euo pipefail
...
# Encode spaces so Docsify resolves folder names like "37. Sudoku Solver".
    ...
    find "$diff" -mindepth 1 -maxdepth 1 -type d -print \
      ...
      | while IFS= read -r dir; do
          ...
          if [ -f "$dir/solution.md" ]; then
            ...
            # No plain solution.md: nest each variant file.
            printf '  - %s. %s\n' "$id" "$title"
