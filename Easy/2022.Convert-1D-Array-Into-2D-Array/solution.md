...
# Complexity
- Time complexity: The time complexity is O(m × n) because we iterate over all elements of the original array exactly once.

- Space complexity: The space complexity is O(m × n) as we are creating a new 2D array with `m * n` elements.
