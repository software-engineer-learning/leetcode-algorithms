...
- **Time complexity**: The time complexity of converting each character and summing the digits is O(n), where `n` is the length of the string.
  - Each subsequent digit sum operation has a time complexity of O(log(sum)), repeated `k-1` times. Since `log(sum)` is relatively small compared to `n`, the overall complexity is approximately O(n · k).
