...
the term (values[i] + i) depends only on `i`, and (values[j] - j) depends only on `j`. This allows us to use a greedy approach to compute the maximum score efficiently.

# Approach

1. Iterate Over values:

- Keep track of the maximum value of (values[i] + i) encountered so far.
...
1. Update `prev_max_score`:
   Update the maximum value of (values[i] + i) as you iterate through the array.
