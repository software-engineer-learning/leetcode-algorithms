...
This is [3903. Smallest Stable Index I](../../Easy/3903.Smallest-Stable-Index-I/solution.md)
with one constraint changed: `n` rises from `100` to $$10^5$$. The statement, the
examples and the value range are word-for-word identical, so the same code solves
both — what changes is that a quadratic solution now matters. At `n = 100` a
recompute-per-index scan is 10^4 steps and passes comfortably; at $$n = 10^5$$ it
is 10^10 steps and will not.

That is the whole reason this variant is rated Medium: it forces the linear
formulation rather than allowing the naive one.
...
Unlike in part I, the linear time bound is genuinely required here: at
$$n = 10^5$$ the quadratic recompute is around $$10^{10}$$ operations. The space
saving from approach 2 remains optional — one $$10^5$$-element array is a few
hundred kilobytes either way — so the packing is still a flourish rather than a
necessity.
...
| `[10^9] * 10^5` | `0` | all `10^9` | `0` | value ceiling at full length |
...
small cases shared with part I — the three examples, every array of length 1 to 4
over `0..3` paired with every `k` in `0..4` (exhaustive), 4000 random arrays, and
adversarial inputs pinned at `10^9`.

Because the brute force is quadratic, full-scale inputs were checked against an
independent linear reference instead: **32** cases at exactly $$n = 10^5$$ —
random, ascending, descending, all-equal at the value ceiling, and a single peak
after a run of zeros — plus inputs whose answer is the first index, the last
index, and none at all. All five implementations agreed on every one, and the Rust
builds were made in debug mode where an arithmetic overflow would panic; none
occurred.
